...
bool RcvTopicsPubs::init(uxrSession *session, uxrStreamId reliable_out_stream_id, uxrStreamId reliable_in_stream_id, uxrStreamId best_effort_in_stream_id, uxrObjectId participant_id, const char *client_namespace)
{
	...
	}
	{
			...
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 18, client_namespace, "/fmu/in/differential_drive_setpoint", "px4_msgs::msg::dds_::DifferentialDriveSetpoint_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_odometry)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 19, client_namespace, "/fmu/in/vehicle_visual_odometry", "px4_msgs::msg::dds_::VehicleOdometry_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_command)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 20, client_namespace, "/fmu/in/vehicle_command", "px4_msgs::msg::dds_::VehicleCommand_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_command)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 21, client_namespace, "/fmu/in/vehicle_command_mode_executor", "px4_msgs::msg::dds_::VehicleCommand_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_trajectory_bezier)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 22, client_namespace, "/fmu/in/vehicle_trajectory_bezier", "px4_msgs::msg::dds_::VehicleTrajectoryBezier_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_trajectory_waypoint)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 23, client_namespace, "/fmu/in/vehicle_trajectory_waypoint", "px4_msgs::msg::dds_::VehicleTrajectoryWaypoint_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_thrust_setpoint)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 24, client_namespace, "/fmu/in/vehicle_thrust_setpoint", "px4_msgs::msg::dds_::VehicleThrustSetpoint_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_torque_setpoint)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 25, client_namespace, "/fmu/in/vehicle_torque_setpoint", "px4_msgs::msg::dds_::VehicleTorqueSetpoint_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(actuator_motors)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 26, client_namespace, "/fmu/in/actuator_motors", "px4_msgs::msg::dds_::ActuatorMotors_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(actuator_servos)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 27, client_namespace, "/fmu/in/actuator_servos", "px4_msgs::msg::dds_::ActuatorServos_", queue_depth);
	}
	{
			uint16_t queue_depth = orb_get_queue_size(ORB_ID(vehicle_global_position)) * 2; // use a bit larger queue size than internal
			create_data_reader(session, reliable_out_stream_id, best_effort_in_stream_id, participant_id, 28, client_namespace, "/fmu/in/aux_global_position", "px4_msgs::msg::dds_::VehicleGlobalPosition_", queue_depth);
	}
