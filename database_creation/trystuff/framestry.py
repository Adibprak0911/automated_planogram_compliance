...
# resized_roi_dir = '/home/user/Documents/automated_planogram_compliance/automated_planogram_compliance-1/database_creation/kurkureframesresized'
...
#         # Draw the bounding rectangle on the image (optional)
...
#         resized_roi = cv2.resize(roi, (w * 2, h * 2))  # Example: Zoom in by doubling the size

#         # Save the resized ROI to a directory
...
# print(f"Resized ROIs saved to {resized_roi_dir}.")
...
def process_image(image_path):
    ...
    if len(contours) > 0:
        ...
        # Resize the ROI to focus/zoom in on the object detected
        resized_roi = cv2.resize(roi, None, fx=2.0, fy=2.0, interpolation=cv2.INTER_LINEAR)
