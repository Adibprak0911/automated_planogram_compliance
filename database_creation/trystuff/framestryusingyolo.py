...
def process_image(image_path):
    ...
    if len(indexes) > 0:
        # Assume only one detection for simplicity (you can modify for multiple detections)
        for i in indexes.flatten():
            x, y, w, h = boxes[i]
            label = str(classes[class_ids[i]])

            # If the detected object is the Kurkure packet, process it further
            if label == 'packet':  # Replace 'packet' with the actual class name if available
                ...
                print(f"No Kurkure packet detected in {image_path}")
