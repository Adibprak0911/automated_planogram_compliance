...
def process_image(image_path):
    ...
    if len(contours) > 0:
        ...
        # Define new bounding box dimensions to keep the object centered
        new_width = w * 2
        new_height = h * 2
        
        # Calculate new top-left corner of the bounding box
        new_x = max(center_x - new_width // 2, 0)
        new_y = max(center_y - new_height // 2, 0)
        
        # Ensure the new bounding box stays within image bounds
        new_x = min(new_x, image.shape[1] - new_width)
        new_y = min(new_y, image.shape[0] - new_height)
        
        # Extract region of interest (ROI)
        roi = image[new_y:new_y + new_height, new_x:new_x + new_width]
